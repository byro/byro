...
feature. The core modules of byro expose signals for different purposes. You
can find their documentation in the [signal list](../signals.md). We also
...
[Django app](https://docs.djangoproject.com/en/dev/ref/applications/) and must
contain plugin metadata, as described below. You will need some boilerplate
code for every plugin to get started. To save your time, we created a
[cookiecutter](https://cookiecutter.readthedocs.io/en/latest/) template that
...
This will ask you some questions and then create a project folder for your
plugin.
...
[Plugins](../../administration/plugins.md)). To get your plugin listed:
...
* declare the byro versions you support as a dependency, for example
  `dependencies = ["byro>=2026.3"]`, so that an incompatible combination fails
...
* open a pull request against `deploy/plugin-catalog.conf` in the byro
  ...
  name, source and repository (maintainers: [Releasing byro](../releasing.md)).

## Signals

byro defines different signals which your plugin can listen for. We will go
into the details of the different signals in the [signal list](../signals.md).
...
!!! warning
    If you define custom URLs and views, you are on your own with checking that
    the user has appropriate permissions. byro ensures that you are dealing
    with an authenticated user, but nothing else.
