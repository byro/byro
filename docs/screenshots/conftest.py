...
def logged_in_client(live_server, client, user):
    ...
    client.find_element_by_css_selector("form input[name=username]").send_keys(user.username)
    client.find_element_by_css_selector("form input[name=password]").send_keys('test_password')
...
def chrome_options(chrome_options):
    ...
    chrome_options.add_argument('window-size=1392x783')
