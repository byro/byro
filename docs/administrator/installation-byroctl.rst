...
script. It downloads ``byroctl`` for the current stable release, verifies it
and starts the installation::

    $ mkdir ~/byro && cd ~/byro
    $ bash -c "$(curl -fsSL https://raw.githubusercontent.com/byro/byro/stable/install.sh)"

Add ``-- --root /path`` to name the directory up front (a relative path is
taken from the current directory), or ``-- --dry-run`` to see what the script
...
* the installation directory, proposing the current one (skipped with
  ``--root``); a directory that already holds files but no byro installation
  has to be confirmed,
